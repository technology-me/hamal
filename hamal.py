...
class HamalError(Exception):                                        # 参数错误类
    pass                                                            # 过
    ...
    if ',' in link_text:                                            # 如果有“，”分隔符
        ...
        if len(link_text_split) == 3:                               # 如果有三项
            ...
            read_text = read_sheet(
                file_name, link_text_split[0], link_text_split[1], link_text_split[2])# 用表读取函数
    else:                                                           # 如果没有“，”分隔符
        read_text = read(file_name, link_text)                      # 用字读取函数
    if read_text[1:] == link_text:                                  # 如果引用自己
        raise HamalError(
            'Do you want to refer to yourself and create infinite recursion?')# 抛出错误
    return(read_text)
...
def view(file_name, value=False):
    ...
    """设置滚动条"""
    scroll.pack(
        side='right', fill=Y)                                       # 滚动条对齐：右；滚动条填充：Y轴
        ...
        if key in python_text.keys():                               # 如果键在json的键中
            if isinstance(python_text[key], str):                   # 如果值是str
                if python_text[key][0] == link[0]:                  # 如果str的值首字母是link首字母（必须是分开的，否则第二个if会并报错）
                    return(_linker(file_name, python_text[key]))    # 运用递归算法，寻求最终值
...
def delete(file_name, key):
    ...
    if python_text == FILE_NOT_FOUND:                               # 读取错误
        raise HamalError("'"+key+"'"+' is not a correct file.')     # 抛出错误
...
def decode(file_name):
    ...
    try:                                                            # 准备测试
        ...
        raise HamalError("File '"+file_name+"' can not decode.")    # 抛出定义
    with open(file_name, 'wb+') as f:                               # 打开文件
...
class Hamal():
    ...
    def change(self, key, in_key, in_value):
        """替换键内字典或列表的指定键或项的内容"""
        return(change(self.file_name, key, in_key, in_value))      # 调用函数
